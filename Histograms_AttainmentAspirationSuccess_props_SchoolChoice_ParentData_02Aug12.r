...
for(pset in 1:1)
{
 ...
 # dat15.sD0_lowAtt <- dat15.success[dat15.success$distance.rounded == 0 & dat15.success$attainment.delta < -10,]
  ...
  asp.dat.upper <- rbind(attAsp0.upper, attAsp1.upper, attAsp2.upper, attAsp3.upper, attAsp4.upper)
  asp.dat.lower <- rbind(attAsp0.lower, attAsp1.lower, attAsp2.lower, attAsp3.lower, attAsp4.lower)
    
  success.dat <- rbind(successAsp0.means, successAsp1.means, successAsp2.means, successAsp3.means, successAsp4.means)
  
  asp.sums <- apply(asp.dat, 1, sum)
  asp.props <- asp.dat / asp.sums
  asp.upper.props <- asp.dat.upper / asp.sums
  asp.lower.props <- asp.dat.lower / asp.sums
  
  success.props <- success.dat / asp.sums
  
#  max.count <- max(asp.dat)
#  max.countR <- max(pretty(c(0, max.count)))

  max.count <- max(asp.props)
  max.countR <- max(pretty(c(0, max.count)))
  
  
  png(paste(pathname, "ParameterSet", pset, "_AttainmentVsAspirationVsSuccess_propBars.png", sep = ""), width = 2000, height = 2000)
  par(cex = 3, lwd = 2)
  barplot(asp.props, col = barcols, beside = T, axes = F, ylim = c(0, max.countR), ylab = "Proportion (of aspiration class)", xlab = "Attainment Change")
  barplot(asp.props, col = "black", density = 10, angle = 45, add = T, beside = T, axes = F)
...
#  error.bar(seq(5.5, 59.5, 6), attAsp4.means, upper = (attAsp4.upper - attAsp4.means), lower = (attAsp4.means - attAsp4.lower), length = 0)
  ...
  text(54, 0.5, "Hatching = Fail")
  
  dev.off()
  
  
  
  pdf(paste(pathname, "ParameterSet", pset, "_AttainmentVsAspirationVsSuccess_propsBars.pdf", sep = ""), width = 10, height = 8)
  par(cex = 1, lwd = 1)
  barplot(asp.props, col = barcols, beside = T, axes = F, ylim = c(0, max.countR), ylab = "Proportion (of aspiration class)", xlab = "Attainment Change")
  barplot(asp.props, col = "black", density = 30, angle = 45, add = T, beside = T, axes = F)
  barplot(success.props, col = barcols, add = T, beside = T, axes = F)
