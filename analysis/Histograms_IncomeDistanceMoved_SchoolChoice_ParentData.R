...
for(pset in 1:1)
{
  ...
  dat99.movedDist0 <- dat99.moved[dat99.moved$distance.rounded == 0,]
  dat99.movedDist1 <- dat99.moved[dat99.moved$distance.rounded == 1,]
  dat99.movedDist2 <- dat99.moved[dat99.moved$distance.rounded == 2|dat99.moved$distance.rounded == 3,]
  dat99.movedDist3 <- dat99.moved[dat99.moved$distance.rounded == 4|dat99.moved$distance.rounded == 5,]
  dat99.movedDist4 <- dat99.moved[dat99.moved$distance.rounded == 6|dat99.moved$distance.rounded == 7,]
  dat99.movedDist5 <- dat99.moved[dat99.moved$distance.rounded == 8|dat99.moved$distance.rounded == 9,]
  ...
  movedDist0.counts <- hist(dat99.movedDist0$hhold.income, incBreaks, plot = F)$counts
  movedDist1.counts <- hist(dat99.movedDist1$hhold.income, incBreaks, plot = F)$counts
  movedDist2.counts <- hist(dat99.movedDist2$hhold.income, incBreaks, plot = F)$counts
  movedDist3.counts <- hist(dat99.movedDist3$hhold.income, incBreaks, plot = F)$counts
  movedDist4.counts <- hist(dat99.movedDist4$hhold.income, incBreaks, plot = F)$counts
  movedDist5.counts <- hist(dat99.movedDist5$hhold.income, incBreaks, plot = F)$counts
  ...
  for(j in 2:length(check))
  {
    ...
    movedDist0.counts <- cbind(movedDist0.counts, hist(dat99.movedDist0$hhold.income, incBreaks, plot = F)$counts)
    movedDist1.counts <- cbind(movedDist1.counts, hist(dat99.movedDist1$hhold.income, incBreaks, plot = F)$counts)
    movedDist2.counts <- cbind(movedDist2.counts, hist(dat99.movedDist2$hhold.income, incBreaks, plot = F)$counts)
    movedDist3.counts <- cbind(movedDist3.counts, hist(dat99.movedDist3$hhold.income, incBreaks, plot = F)$counts)
    movedDist4.counts <- cbind(movedDist4.counts, hist(dat99.movedDist4$hhold.income, incBreaks, plot = F)$counts)
    movedDist5.counts <- cbind(movedDist5.counts, hist(dat99.movedDist5$hhold.income, incBreaks, plot = F)$counts)
  ...
  movedDist0.means <- apply(movedDist0.counts, 1, mean, na.rm = T)
  movedDist1.means <- apply(movedDist1.counts, 1, mean, na.rm = T)
  movedDist2.means <- apply(movedDist2.counts, 1, mean, na.rm = T)
  movedDist3.means <- apply(movedDist3.counts, 1, mean, na.rm = T)
  movedDist4.means <- apply(movedDist4.counts, 1, mean, na.rm = T)
  movedDist5.means <- apply(movedDist5.counts, 1, mean, na.rm = T)
  ...
  moved.dat <- rbind(movedDist0.means, movedDist1.means, movedDist2.means, movedDist3.means, movedDist4.means, movedDist5.means)
  ...
  png(paste(pathname, "ParameterSet", pset, "_HHoldIncomeVsDistanceVsMoved_Bars.png", sep = ""), width = 3500, height = 2000)
  par(cex = 3, lwd = 2)
